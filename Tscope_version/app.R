# shiny app to analyse the data produced by the original STOP-IT program. 
...
server <- function(input, output) {
  data<-eventReactive(input$process,({
      do.call("rbind", (lapply(input$csvs$datapath, 
                               function(i){
                                dd <- read.table(i, header=TRUE, na.strings=c("null"), colClasses=c("correct"="factor"))
                                dd$participantID <- rep(subset(input$csvs, datapath==i, select = c(name)), nrow(dd))                                        
                                dd}
                               )))
  }))
  ...
  processed<-reactive({
    ...
    for (i in nsubjects){
      ...
      dataset<-reactive({subset(data(), data()$participantID == i)})

      # stop-signal trials
      stopsignal<-reactive({subset(dataset(), dataset()$signal == 1)})
      Nstop <- nrow(stopsignal())

      tmp <- ifelse(stopsignal()$respons == 0, 0, 1)
      presp <- mean(tmp)
      ssd <- round(mean(stopsignal()$ssd))

      stopsignal.resp.trials <- reactive({subset(stopsignal(), stopsignal()$respons != "0")})
      usRT <- round(mean(stopsignal.resp.trials()$rt))

      # go trials
      go<-reactive({subset(dataset(), dataset()$signal == 0)})
      Ngo <- nrow(go())

      go.resp.trials <- reactive({subset(go(), go()$respons != 0)})
      goRT_all <- round(mean(go.resp.trials()$rt))
      goRT_sd <- round(sd(go.resp.trials()$rt))

      goRT.max <- max(go.resp.trials()$rt)
      goRT.adj <- ifelse(go()$respons == 0, goRT.max, go()$rt)
      nth <- as.vector(round(quantile(goRT.adj, probs = presp, type = 6)))

      go.correct.trials <- reactive({subset(go(), go()$correct == 2)})
  ...
  ss_presented	<- data()$signal
  tmp	<- ifelse(data()$correct == '2', 1, 0)
  inhibited <- ifelse(data()$signal == 0, -999, tmp)
  tmp <- data()$ssd
  ssd	<- ifelse(data()$signal == 0, -999, tmp)
  tmp <- data()$rt
  rt	<- ifelse(data()$rt == 0, -999, tmp)
